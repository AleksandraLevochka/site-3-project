var jsonData = [
    {
      ...
      {
        "Borough": "X",
        "Name": "Rev. T. Wendell Foster Park",
      ...
      {
        "Borough": "X",
        "Name": "Van Cortlandt Park Basketball Court",
        "Location": "Broadway between Van Cortlandt Park South and West 242nd Street (next to the stadium)",
        "Num_of_Courts": null,
        "Accessible": "N",
        "lat": "40.8853",
        "lon": "-73.891"
      ...
      },
      {
        "Borough": "Q",
        "Name": "Big Bush Park",
        "Location": "61st Street and Brooklyn Queens Expressway",
        "Num_of_Courts": "1",
        "Accessible": null,
        "lat": null,
        "lon": null
      ...
      },
      {
        ...
        "Location": "Back of Horace Greeley Middle School (I.S. 10), between 45th and 46th Street",
        "Num_of_Courts": "2",
        "Accessible": null,
        "lat": null,
        "lon": null
      ...
      },
      {
        ...
        "Location": "12th Street between Second Avenue and Third Avenue",
        "Num_of_Courts": "1",
        "Accessible": null,
        "lat": null,
        "lon": null
      ...
      },
      {
        ...
        "Location": "Location: Franklin Avenue and Willoughby Avenue",
        "Num_of_Courts": "1",
        "Accessible": null,
        "lat": null,
        "lon": null
      ...
      },
      {
        ...
        "Location": "Viele Ave and Casanova St",
        "Num_of_Courts": "2",
        "Accessible": "N",
        "lat": null,
        "lon": null
      },
      {
        ...
        "Location": "Sackman St between Truxton St & Fulton St",
        "Num_of_Courts": "3",
        "Accessible": null,
        "lat": "40.6783",
        "lon": "-73.9055"
      ...
      {
        "Borough": "X",
        "Name": "Woodlawn Basketball Courts",
        "Location": "Van Cortlandt Park East between Kepler Avenue and Oneida Avenue",
        "Num_of_Courts": "2",
        "Accessible": null,
        "lat": null,
        "lon": null
      ...
      {
        "Borough": "X",
        "Name": "Classic Playground Basketball Courts",
      ...
      },
      {
        "Borough": "X",
        "Name": "Ewen Park",
        "Location": "Riverdale to Johnson avenues, south of West 232nd Street",
        "Num_of_Courts": "2",
        "Accessible": null,
        "lat": "40.8831",
        "lon": "-73.9099"
...
function createBasketBall(ball_number, boroughName, pageURL) {
  ...
  let ballContainer = document.createElement("a"); // creat div for each part
  let ball1 = document.createElement("div");
  let inner =  document.createElement("div");
  let line =  document.createElement("div");
  let lineTwo =  document.createElement("div");
  let label = document.createElement("div"); // Create a new element for the label

  ballContainer.href = pageURL; // Set the href attribute to the pageURL
  ballContainer.classList.add("ball-container"); // add needed classes to each div
  ball1.classList.add("ball1");
  ball1.setAttribute("id", `animation_${ball_number}`); //  give each a unique ID
  inner.classList.add("inner");
  line.classList.add("line");
  lineTwo.classList.add("line", "line--two");
  ballContainer.setAttribute("id", ball_number); //  give each a unique ID

  inner.appendChild(line); // append divs from inmost out
  inner.appendChild(lineTwo);
  ball1.appendChild(inner);
  ballContainer.appendChild(ball1);
  docBody.appendChild(ballContainer); // append all to the document body

  // Create and position the label above the ball
  label.textContent = boroughName;
  label.classList.add("borough-label");
  ballContainer.appendChild(label); // Append the label to the ball container
}

// call createBasketBall five times with increasing number, i
for(let i = 0; i < 5; i++) { 
  let borough = jsonData[i].Borough;
  let boroughFullName = boroughMap[borough]; // to get a full name
  let pageURL = pageURLs[i]; // Get the corresponding page URL
  createBasketBall(`basketball_${i}`, boroughFullName, pageURL); // template literal
}

let basketball_0 = document.getElementById("basketball_0"); // one ball for each borough (Brooklyn)
let basketball_1 = document.getElementById("basketball_1"); // (Manhattan)
let basketball_2 = document.getElementById("basketball_2"); // (Queens)
let basketball_3 = document.getElementById("basketball_3"); // (Long Island)
let basketball_4 = document.getElementById("basketball_4"); // (Bronx)

let px_0 = .8; // PLAY WITH THIS VALUE FOR SCALE
let px_1 = .9; // PLAY WITH THIS VALUE FOR SCALE
let px_2 = .9; // PLAY WITH THIS VALUE FOR SCALE
let px_3 = 1.9; // PLAY WITH THIS VALUE FOR SCALE
let px_4 = .9; // PLAY WITH THIS VALUE FOR SCALE

for(let data of jsonData) {
  let borough = data.Borough;

  // x = y setting a value
  // x == y testing if two values are the same (returns a boolean value TURE or FALSE)
  // x === y same as above but strict

  //   B M Q R X
  if(borough === "B") {
    basketball_0.style.transform = `scale(${px_0}%,${px_0}%)`;
    px_0 += .8; // MAKE IT THE SAME AS THIS ONE
  } else if(borough === "M") {
    basketball_1.style.transform = `scale(${px_1}%,${px_1}%)`;
    px_1 += .9; // MAKE IT THE SAME AS THIS ONE
  } else if(borough === "Q") {
    basketball_2.style.transform = `scale(${px_2}%,${px_2}%)`;
    px_2 += .9; // MAKE IT THE SAME AS THIS ONE
  } else if(borough === "R") {
    basketball_3.style.transform = `scale(${px_3}%,${px_3}%)`;
    px_3 += 1.9; // MAKE IT THE SAME AS THIS ONE
  } else if(borough === "X") {
    basketball_4.style.transform = `scale(${px_4}%,${px_4}%)`;
    px_4 += .9; // MAKE IT THE SAME AS THIS ONE
  }
}

document.querySelectorAll('.ball1').forEach((ball, index) => {
  ball.style.top = `${index * 5}px`; // Adjust the value as needed
});
...
// function courtsJSON() { // declare anamed function to iterate the json
//     const uniqueZips = []; //declare an empty array
//     jsonData.forEach(courts => { // iterate the json named jsonData using variable "courts"
//         let currentCourtBorough = courts.Borough;
//         currentCourtBorough.forEach(thisZip => {
//             if (!uniqueZips.includes(thisZip)) {
//                 uniqueZips.push(thisZip); // add to the array
//                 console.log("zip list: " + uniqueZips);
//                 let newParagraph = document.createElement("p"); // create a new paragraph
//                 newParagraph.setAttribute("id", thisZip);
//                 newParagraph.style.fontSize = "36px";
//                 newParagraph.innerText = thisZip;
//                 document.body.appendChild(newParagraph);
//             } else {
//                 let existingParagraph = document.getElementById(thisZip);
//                 let existingParagraphSize = existingParagraph.style.fontSize;
//                 let sizeNumberOnly = existingParagraphSize.split("px")[0];
//                 let newsizeNumber = Number(sizeNumberOnly) + 4;
//                 existingParagraph.style.fontSize = `${newsizeNumber}px`;
//             }
//         })
//     });
// }
...
// Group courts by borough
// var groupedCourts = {};
// jsonData.forEach(function(court) {
//     var borough = court.Borough;
//     if (!groupedCourts[borough]) {
//         groupedCourts[borough] = [];
//     }
//     groupedCourts[borough].push(court);
// });

// console.log(groupedCourts)

// // Function to display courts for a specific borough
// function displayCourts(borough) {
//     var courts = groupedCourts[borough];
//     if (courts) {
//         var result = "<h2>" + getBoroughName(borough) + "</h2>"; // Display borough name
//         courts.forEach(function(court) {
//             result += "<p>" + court.Name + "</p>"; // Display court name
//         });
//         document.getElementById("courts-container").innerHTML += result; // Display on the page
//     }
// }

// // Function to get the full borough name based on the initial
// function getBoroughName(boroughInitial) {
//     switch (boroughInitial) {
//         case 'X':
//             return 'Bronx';
//         case 'B':
//             return 'Brooklyn';
...
// // Display courts for each borough
// for (var borough in groupedCourts) {
//     displayCourts(borough);
// }